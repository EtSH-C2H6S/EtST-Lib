...
public class EnergeticAttack extends EtSTBaseModifier {
    ...
    public boolean onProjectileHitEntity(ModifierNBT modifiers, ModDataNBT persistentData, ModifierEntry modifier, Projectile projectile, EntityHitResult hit, @Nullable LivingEntity attacker, @Nullable LivingEntity target, boolean notBlocked) {
        if (attacker!=null&&target!=null&&!projectile.level().isClientSide){
            if (projectile instanceof AbstractArrow arrow&&!arrow.isCritArrow()) return super.onProjectileHitEntity(modifiers, persistentData, modifier, projectile, hit, attacker, target, notBlocked);
